...
Inductive envSigma (n : frame_id) (Gamma Gamma' : env) : Prop :=
  | EnvSigma :
      (forall x t, Gamma (env_var (SV x)) = Some t ->
                      Gamma' (env_var (SV x)) = Some t) ->
      (forall x n' t, Gamma (env_var (DV (x, n'))) = Some t ->
                      Gamma' (env_var (DV (x, n))) = Some t) ->
      (forall l t, Gamma (env_loc l) = Some t ->
                   Gamma' (env_loc l) = Some t) ->
      (forall x, Gamma x = None -> Gamma' x = None) ->
...
Inductive hasType (P : program) (Gamma : env) : expr -> ty -> Prop := 
  ...
  | T_ConsumeField :
      forall x f c t r fs,
        ...
        fields P (TClass c) = Some fs ->
        fieldLookup fs f = Some (Field f t r) ->
        P ; Gamma |- EConsumeField x f \in t
  | T_Update :
  ...
  | T_Let :
      forall x frame e body t t',
        P ; Gamma |- e \in t ->
        P ; extend Gamma (env_var (SV x)) t |- body \in t' ->
        no_locks body ->
        P ; Gamma |- ELet x frame e body \in t'
  | T_Cast :
      forall e t1 t2,
        P ; Gamma |- e \in t2 ->
        subtypeOf P t2 t1 ->
  ...
  | T_RLock :
      forall x t2 fs r e t,
        ...
        P ; Gamma |- e \in t ->
        no_locks e ->
        P ; Gamma |- ERlock x r e \in t
  | T_WLocked :
      forall l t2 r e t,
        P ; Gamma |- (EVal (VLoc l)) \in t2 ->
        P ; Gamma |- e \in t ->
        P ; Gamma |- EWlocked (l, r) e \in t
  | T_RLocked :
      forall l t2 r e t,
        P ; Gamma |- (EVal (VLoc l)) \in t2 ->
        P ; Gamma |- e \in t ->
        P ; Gamma |- ERlocked (l, r) e \in t
  | T_Assert :
      forall x y e t2 t,
        P ; Gamma |- (EVar x) \in t2 ->
        P ; Gamma |- (EVar y) \in t2 ->
        P ; Gamma |- e \in t ->
        no_locks e ->
        P ; Gamma |- EAssert x y e \in t
  where " P ';' Gamma '|-' expr '\in' ty " := (hasType P Gamma expr ty).

Tactic Notation "hasType_cases" tactic(first) ident(c) :=
  first;
  [ Case_aux c "T_Var" 
  | Case_aux c "T_Consume"  
  | Case_aux c "T_Loc" 
  | Case_aux c "T_Null" 
  | Case_aux c "T_New" 
  | Case_aux c "T_Call" 
  | Case_aux c "T_Select" 
  | Case_aux c "T_ConsmueField" 
  | Case_aux c "T_Update" 
  | Case_aux c "T_Let"  
  | Case_aux c "T_Cast"
  | Case_aux c "T_Par"
  | Case_aux c "T_WLock"
  | Case_aux c "T_RLock"
  | Case_aux c "T_WLocked"
  | Case_aux c "T_RLocked"
  | Case_aux c "T_Assert"
  ].
