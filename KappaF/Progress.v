...
Proof with eauto using step.
  ...
  inverts wfT as Hfree HframeFresh hasType wfLs wfL.
  hasType_cases(induction hasType) Case; 
    ...
    (* Invert the frame freshness assumption *)
    inv HframeFresh...
  + Case "T_New".
    right.
  ...
  + Case "T_Update".
    ...
    - SCase "e can step". 
      ...
      inv eSteps... 
  + Case "T_Let".
    remember (fun e : expr => ELet x frame e body) as ctx.
    assert (is_econtext ctx). subst. apply EC_Let.
    assert (wfL': wfLocking H (T_Thread Ls e))
      by (apply wfLocking_econtext with ctx; crush).
    replace (ELet x frame e body) with (ctx e) by crush...
    assert (IH: threads_done (T_Thread Ls e) \/ 
                cfg_blocked (H, V, n, T_Thread Ls e) \/
                exists cfg', P / (H, V, n, T_Thread Ls e) ==> cfg')...
  ...
  + Case "T_WLock".
    ...
    assert(wfLs: wfHeldLocks H Ls) 
      by (inv wfL; eauto).
    destruct status as [|m]; destruct HIn as [HIn|HnotIn]...
    - SCase "Thread holds rlock". 
      eapply wfHeldLocks_taken in HIn... 
      rewrite_and_invert.
    - SCase "rlocked". 
      destruct m; eauto using step with arith.
  + Case "T_RLock".
    destruct v...
    inv hasType.
    wfEnvLookup.
  ...
  + Case "T_RLocked".
    assert (wfL': wfLocking H (T_Thread Ls e))
      by eauto using wfLocking_econtext.
    inv hasType1.
    wfEnvLookup. 
    assert (wfRl: wfRLocks H (T_Thread Ls (ERlocked (l, r) e))) 
      by (inv wfL; eauto).
    destruct wfRl as [wfRl]. 
    simpls. 
    assert (Heq: (l, r) = (l, r) \/ In (l, r) (rlocks e))...
    pose proof (wfRl l r Heq) as (c' & F' & RL' & m & Hlookup' & HRL & Hle).
    rewrite_and_invert.
    assert (IH: threads_done (T_Thread Ls e) \/ 
                cfg_blocked (H, V, n, T_Thread Ls e) \/
                exists cfg', P / (H, V, n, T_Thread Ls e) ==> cfg')...
...
Proof with eauto.
  ...
  + Case "T = Async T1 T2 e".
    ...
    inverts wfL as wfWl HWl wfRl Hdisj wfL1 wfL2.
    right. right.
