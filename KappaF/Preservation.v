...
Proof with eauto using subtypeOf with env.
  ...
  inverts wfT as Hfree HframeFresh hasType.
  gen cfg'.
  ...
  + Case "T_Consume". 
    ...
    - econstructor...
      * introv envLookup.
        wfEnvLookup. wfEnvLookup. rewrite_and_invert.
        case_extend...
      * introv Hneq. 
        {destruct (id_eq_dec p x).
         + subst. simpl_extend_hyp.
         + rewrite extend_neq in Hneq...
           inv wfV...
        }
      * {econstructor.
         + introv Hle. unfold fresh.
           case_extend.
           - inv wfV. inv H5. 
             eapply H6 with (frame' := frame') in Hle. 
             inv Hle. rewrite_and_invert.
           - inv wfV. inv H5. apply H6...
         + introv Hle. unfold fresh.
           case_extend.
           - inv wfV. inv H5.
             eapply H7 in Hle.
             inv Hle. rewrite H8 in H4. congruence.
           - inv wfV. inv H5. apply H7...
        }
    - wfEnvLookup. rewrite_and_invert.
  + Case "T_New".
  ...
  + Case "T_Call".
    ...
    - SCase "EvalCall". clear IHhasType.
      ...
      exists (extend 
                (extend 
                   Gamma (env_var (DV (dthis, n))) (TClass c)) 
                (env_var (DV (DVar n, n))) t2).
      assert (fresh Gamma (env_var (DV (dthis, n))))...
      assert (fresh Gamma (env_var (DV (DVar n, n))))...
      split...
      * { econstructor; auto.
          ...
          + lookup_forall as wfMtd.
            ...
            - apply frameFresh_subst...
              apply frameFresh_subst... 
              eapply frameFresh_sigma...
              eapply frameFresh_1...
              omega.
              eapply frameFresh_static...
            - unfold dthis. 
              assert (0 <> n) by omega.
  ...
  + Case "T_ConsmueField".
    ...
    inv Hstep; 
      try(malformed_context); try(inv_eq);
      try(preservation_context_tactic 2).
    - SCase "EvalConsumeField". clear IHhasType.
      wfEnvLookup. rewrite_and_invert.
      inverts hasType as Vlookup envLookup Hsub.
      assert (t2 = TClass c); subst...
      assert (c0 = c) 
  ...
  + Case "T_Let". 
    ...
    assert (wfLocking H (T_Thread Ls e))
      by (apply wfLocking_econtext with (ctx := ctx_let x frame body); eauto).
    inv Hstep; 
      try(malformed_context); try(inv_eq);
      try(preservation_context_tactic 2).
    - SCase "EvalLet". clear IHhasType1. clear IHhasType2.
      exists (extend Gamma (env_var (DV (DVar n, frame))) t). 
      split...
      * { econstructor; auto. 
          + eapply wfHeap_invariance
            with (Gamma := Gamma);
          ...
          + eapply wfVars_extend... 
            eapply wfVars_ge...
          + econstructor; 
            eauto using frameFresh_subst, frameFresh_ge, hasType_subst with env;
            autorewrite with freeVars... 
        }
  ...
  + Case "T_WLock".
    ...
    - SCase "V x = l".
      ...
      * eapply wfHeap_wfRegionLocks with (P := P) in H22 as []...
        econstructor... 
    - inv Hstep;
      try(malformed_context); try(inv_eq);
      try(preservation_context_tactic 2);
  ...
  + Case "T_WLocked".
    ...
    inv Hstep; 
      try(malformed_context); try(inv_eq);
      try(preservation_context_tactic 2).
    - SCase "EvalWlock_Release". clear IHhasType1. clear IHhasType2. 
      exists Gamma. split...
      econstructor...
      eapply wfHeap_update...
      * eapply wfHeap_wfFields...
      * eapply wfHeap_wfRegionLocks with (P := P) in H8 as []...
        econstructor... 
  + Case "T_RLocked".
    inv HframeFresh.
    inv hasType1.
...
Proof with eauto using wfConfiguration.
  ...
  + Case "T = T_Async T1 T2 e".
    inverts wfT as Hfree HframeFresh hasType wfT1 wfT2.
